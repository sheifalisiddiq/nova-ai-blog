...
const INITIAL_POSTS: BlogPost[] = [
...

  {
    ...
    week: 8,
    date: 'Week 8',
    title: 'Week 8: SSL/TLS Inspection, IDS Validation, and Dashboard Stabilization',
    concept:
      'This week focused on integrating, auditing, testing, and stabilizing several major NOVA capabilities rather than simply adding isolated features. The main work involved validating the SSL/TLS Certificate Inspector, auditing the current IDS logic and detection presentation layers, fixing repeated passive-state dashboard spam, reducing false-positive incident escalation, and improving the consistency of alert and report generation. Overall, this week became a debugging, refinement, and demo-readiness phase.',
    rationale:
      'At this stage, NOVA needed more than new modules. It had to behave consistently across monitoring, detection, escalation, reporting, and presentation. That meant checking whether features that appeared complete were actually reliable in practice, whether passive observations were being presented too aggressively, and whether incident promotion logic was strict enough to prevent weak evidence from turning into misleading critical alerts.',
    details: {
      significance:
        'Week 8 strengthened NOVA at the system level. Instead of only adding features, the work focused on validating existing capabilities, removing misleading dashboard behavior, tightening detection escalation, and ensuring the project behaves more like a dependable security tool than a fragile prototype. By the end of the week, the dashboard was cleaner, the alert path was more disciplined, the SSL/TLS inspector was working correctly, and the project was substantially more stable and demo-ready.',
      approach: {
        capture:
          'Planned Integration + Dashboard Stabilization: The intended implementation order was SSL/TLS Certificate Inspector first, Expanded IDS Logic second, and better detection presentation in the dashboard and reports third. That order was chosen because it offered faster visible improvement, easier Codex integration, and lower risk of destabilizing the system. Separate prompts were prepared for each feature area to avoid one oversized implementation and reduce debugging complexity. During testing, the dashboard was found to be showing repeated incident-like cards even during passive monitoring, with repeated low-severity entries, no real attack type, repeated source details, and generic recommendations such as "Monitor" or "Observe." At first this looked like a recent-incidents deduplication problem, but the issue was traced more directly to the live status rendering path. Passive or no-threat states were being displayed too similarly to true incidents. I adjusted passive-state rendering into a compact neutral monitoring block, removed repeated live status rendering behavior, and stabilized the display so it appears once instead of building a wall of repeated incident-style content. The resulting neutral view now reflects states such as Current State: Monitoring, Current Threat: None, Source: N/A, and Next Step: Observe.',
        ai:
          'IDS Validation + False-Positive Reduction: Rather than blindly re-adding Expanded IDS Logic, I audited the existing codebase and confirmed that NOVA already contains a substantial app-side hybrid IDS layer with live feature extraction, rule-based evaluation, attack classification, explanation generation, hybrid scoring, severity mapping, promotion gating, alert gating, evidence counters, hysteresis and recovery logic, incident confirmation, deduplication and cooldown behavior, false-positive suppression, and filtered incident rendering. Supported categories already include Port Scan Suspected, Suspicious Multi-Port Activity, Dominant Source Burst, Repeated Connection Pattern, Traffic Spike, General Anomalous Traffic, and None. The bigger issue was escalation discipline. NOVA had generated and emailed what appeared to be a critical port-scan incident even when the evidence was too weak to justify it. That exposed inconsistency across is_threat, classification, severity mapping, logging, report generation, and email sending. I tightened shared promotion gating, added shared alert and report gating, grounded severity mapping more carefully, strengthened incident confirmation rules, prevented weak passive observations from becoming real incidents, removed duplicated bypass logic still affecting the live loop, and gated manual BLOCK IP and REPORT behavior as well. As a result, weak traffic is now much less likely to become a false critical incident, report, or email.',
        ui:
          'SSL/TLS Validation + Reporting Audit: The SSL/TLS Certificate Inspector was tested against targets such as google.com:443 and initially returned incorrect results including Hostname Match = No and Status = Critical. The root cause was that the code relied on ssl.match_hostname(...), but the runtime did not expose that attribute and the missing function was being treated incorrectly as a certificate mismatch. I fixed this by adding a fallback hostname validator that uses built-in matching when available, otherwise manually checks SAN entries, falls back to CN if necessary, and includes wildcard-aware matching. Retesting then behaved correctly for google.com, openai.com, and github.com, while failure cases also worked properly: notarealhost12345.com:443 produced DNS failure, google.com:80 produced TLS handshake failure, expired.badssl.com:443 detected an expired certificate, and wrong.host.badssl.com:443 detected hostname mismatch. In parallel, I audited detection presentation and reporting rather than re-adding Feature 3 from scratch. The existing layer already included readable detection summary cards, severity display, source IP display, attack type display, short explanation text, analyst-style summary text, recommended action text, structured PDF report generation, email and report workflows, passive-state filtering, and separation between passive monitoring and true incidents. Reusable helpers such as format_detection_summary(...) and build_incident_report_block(...) were also confirmed. The main remaining gap was polish and consistency rather than missing core functionality, and the SSL/TLS inspector can now be considered complete.'
      }
    },
    reflections:
      'Week 8 showed that meaningful engineering progress is not just about adding new modules. A major part of real system development is auditing what already exists, finding hidden logic flaws, reducing false alarms, and improving clarity and reliability. This week made NOVA much cleaner, more stable, less prone to false-positive escalation, more accurate in passive-state presentation, more reliable in SSL/TLS inspection, and much more defensible as a final year project demonstration.',
    tags: ['SSL/TLS', 'Dashboard Stabilization', 'IDS Validation', 'False Positive Reduction', 'Reporting', 'System Integration'],
    comments: []
  }
];
